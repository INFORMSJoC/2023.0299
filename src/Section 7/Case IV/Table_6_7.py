...
#     m.addConstr( slack_t >= sum(x[i] for i in range(x_random_size)))
    ...
    m.update()
    m.addConstr( theta*(epsilon**(1-1/q))*slack_t -(epsilon)*gamma - s.sum()/float(random_size)  <=0)
    m.update()
    # Solve the problem
    m.update()
...
#     m.addConstrs(s[j] >= gamma for j in range(random_size))
    ...
    m.addConstr(theta*lambda_1*(epsilon**(1-1/q)) + s.sum()/float(random_size) + epsilon*gamma <= 0)
    m.update()
...
def Model_alsox_sharp():
    ...
    # Add objective    
    m.setObjective(theta*(epsilon**(1-1/q))*slack_t -(epsilon)*gamma - s.sum()/float(random_size), GRB.MINIMIZE)
    m.update()
    # Add constraints
#     m.addConstrs(  s[i] >= beta for i in range(random_size))
    m.update()
...
m.update()
m.addConstr( theta*(epsilon**(1-1/q))*slack_t + s.sum()/float(random_size) + epsilon*beta <=0)
m.update()
# Solve the problem
# m.update()
...
#     m.addConstr( slack_t >= sum(x[i] for i in range(x_random_size)))
    ...
    m.addConstr( theta*(epsilon**(-1/q))*slack_t + sum(xi_2[j]*x[j] for j in range(x_random_size))  <= b_2)
...
#     m.addConstr( theta/epsilon*slack_t + sum(xi_2[j]*x[j] for j in range(x_random_size))  <= b_2)
    ...
    m.addConstrs( theta*(epsilon**(-1/q))*slack_t + sum(xi[i][j]*x[j] for j in range(x_random_size)) <= b[i] + Big_M_coefficient_VaR[i]*(1-z_appro[i])  for i in stengthen_difference)
    m.update()
    m.addConstr(z_appro.sum() >= random_size - math.ceil(random_size*(epsilon)))
    m.update()
...
#     m.addConstrs(  sum(xi[i][j]*x[j] for j in range(x_random_size)) <= b[i] + Big_M_updated[i]*(1-z[i])  for i in stengthen_difference)
    ...
    m.addConstrs( theta*(epsilon**(-1/q))*lambda_1[0] + sum(xi[i][j]*x[j] for j in range(x_random_size)) <= b[i] + Big_M_coefficient_VaR[i]*(1-z_appro[i])  for i in stengthen_difference)
    m.update()
...
#     m.addConstrs(s[j] >= gamma for j in range(random_size))
    ...
    m.addConstr(theta*(epsilon**(1-1/q))*lambda_1 + s.sum()/float(random_size) + epsilon*gamma[0] <= 0)
    m.update()
...
#     m.addConstrs( theta/epsilon*lambda_1[0] + sum(xi[i][j]*x[j] for j in range(x_random_size)) <= b[i] + Big_M_updated[i]*(1-z_appro[i])  for i in stengthen_difference)
    ...
    m.update()
    m.addConstr(lambda_1[0]*theta*(epsilon**(1-1/q)) - epsilon*gamma[0] - quicksum(y[i] for i in range(random_size))/float(random_size)<=0)
    m.update()
...
best_upper_bound = min(final_big_m[0],value_alsox_sharp)
...
final_Gap = abs(best_upper_bound-best_lower_bound)/abs(best_lower_bound)*100


# In[116]:


print(f'The final gap of model_our_method is exactly {final_Gap:.5f}.') 
...
#     m.addConstrs( theta*(epsilon**(-1/q))*slack_t + sum(xi[i][j]*x[j] for j in range(x_random_size)) <= b[i] + Big_M_coefficient_VaR[i]*(1-z_appro[i])  for i in range(random_size))
#     m.update()
#     m.addConstr(z_appro.sum() >= random_size - math.ceil(random_size*(epsilon)))
#     m.update()
