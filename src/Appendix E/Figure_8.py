...
filename = '1-6-multi-500-5.txt'
f = open("1-6-multi-500-5.txt", "r")
a = []
for line in f:
    a.append(line)
...
#     m.addConstr(slack_t*slack_t >= sum(x[i]*x[i] for i in range(x_random_size)))
    ...
    m.addConstrs(sum(xi[i][k*x_random_size+j]*x[j] for j in range(x_random_size))  <= b[k] + s[i] for i in range(random_size) for k in range(knapsack_size) )
    
#     m.addConstrs( theta*slack_t + g_x_xi[i] <= b[i] + s[i] for i in range(random_size))
    m.update()
...
#     m.addConstrs( slack_t + g_x_xi[i] <= b[i] + s[i] for i in range(random_size))
#     con1=m.addConstr( theta*slack_t+g_x_xi[0] <= b[0] )
#     con1=m.addConstr( g_x_xi[0] <= b[0] )
    con1= m.addConstrs(sum(xi[0][j]*x[j] for j in range(x_random_size))  <= b[k] for k in range(knapsack_size) )
    # Solve the problem
    m.update()
    ...
    for i in range(1,random_size):   
        m.remove(con1)
#         con1=m.addConstr( slack_t+g_x_xi[i]  <= b[i] )
        con1 = m.addConstrs(sum(xi[i][k*x_random_size+j]*x[j] for j in range(x_random_size))  <= b[k] for k in range(knapsack_size) )
    
#         m.addConstr( g_x_xi[i]  <= b[i] )
        m.update()
        m.params.OutputFlag=0    
        m.optimize()
        kkk = m.getAttr('x', x)
...
sorted_array=np.sort(quantile_result)
...
m.addConstrs(sum(xi[i][k*x_random_size+j]*mu[j,i] for j in range(x_random_size))  <= b[k]*z[i] for i in range(random_size) for k in range(knapsack_size) )
...
#delta_1 = CVAR/1000
delta_1=1e-4
# t1 =  max(v_q,lower_bound_relaxation)
t1 =  v_q
t2 =  CVAR
delta_t = t2-t1
t = (t1+t2)/2.0
...
# heuristics
print ("Begin to solve heuristics")
start=time.time()
m = Model()
...
print(f'The objective of alsox is {value_alsox:.5f}.')
...
subset_indice = np.argsort(quantile_result)[random_size-2*violation:random_size]
...
m.addConstrs(sum(xi[i][k*x_random_size+j]*mu[j,i] for j in range(x_random_size))  <= b[k] for i in stengthen_difference for k in range(knapsack_size) )
...
for i in range(0,math.ceil(violation*0.1)):       
...
for i in range(0,math.ceil(violation*0.3)):       
...
#     big_m_coefficient = []
#     for k in range(knapsack_size):
#         eta_value=[]
#         for i in stengthen_difference:
#             new_xi_i = xi[i][k*x_random_size:(k+1)*x_random_size]
#             new_xi_j = xi[j][k*x_random_size:(k+1)*x_random_size]
#             if sum(new_xi_i) <= b[k]:
#                 eta_value.append(max(sum(new_xi_j) - b[k],0))
#     #             print("Special:",i)
#             else:  
#                 division_sort = new_xi_i[np.argsort(np.divide(new_xi_j,new_xi_i))][::-1]

#                 division_index = np.argsort(np.divide(new_xi_j,new_xi_i))[x_random_size-np.where(division_sort.cumsum() > b[k])[0][0]:x_random_size]

#                 eta_value.append(max(sum(new_xi_j[division_index]) - b[k],0))
# #             print(i)
#         big_m_coefficient.append(eta_value[np.argsort(eta_value)[random_size-math.floor(random_size*epsilon)+1]])
#     Big_M_updated[j]=max(big_m_coefficient)
...
# Initialize Big_M_updated as a dictionary
Big_M_updated = {}

for j in stengthen_difference:
    big_m_coefficients_for_j = []  # List to hold big_m_coefficient values for each k for a particular j
    for k in range(knapsack_size):
        eta_value = []
        for i in stengthen_difference:
            new_xi_i = xi[i][k*x_random_size:(k+1)*x_random_size]
        ...
        # Instead of taking max, append the sorted eta_value based on your criterion
        big_m_coefficients_for_j.append(eta_value[np.argsort(eta_value)[random_size-math.floor(random_size*epsilon)+1]])
    
    # Store the list of coefficients for this j
    Big_M_updated[j] = big_m_coefficients_for_j
...
#         g_x_xi[i] = g_x  
    ...
    m.addConstrs(sum(xi[i][k*x_random_size+j]*x[j] for j in range(x_random_size))  <= b[k]+ Big_M_updated[i][k]*z[i] for k in range(knapsack_size) for i in stengthen_difference)
...
# #             print(i)
#         big_m_coefficient.append(eta_value[np.argsort(eta_value)[random_size-math.floor(random_size*epsilon)+1]])
#     Big_M_strength_only.append(max(big_m_coefficient))
...
# Assuming the necessary variables like 'random_size', 'knapsack_size', 'x_random_size', 'b', 'xi', 'epsilon' are defined

start = time.time()

# Initialize Big_M_strength_only as an empty dictionary
Big_M_strength_only = {}
...
for j in range(random_size):
    ...
    for k in range(knapsack_size):
        ...
        # Append the sorted eta_value based on your criterion
        big_m_coefficients_for_j.append(eta_value[np.argsort(eta_value)[random_size-math.floor(random_size*epsilon)+1]])
    
    # Use j as a key to store the list of coefficients
    Big_M_strength_only[j] = big_m_coefficients_for_j
...
#     slack_t = m.addVar(lb=0,name="slack_t")
    ...
    m.addConstrs(sum(xi[i][k*x_random_size+j]*x[j] for j in range(x_random_size))  <= b[k]+ Big_M_strength_only[i][k]*z[i] for k in range(knapsack_size) for i in range(random_size))
    
    m.update()
#     m.addConstr(slack_t*slack_t >= sum(x[i]*x[i] for i in range(x_random_size)))
    m.addConstr(z.sum() <= math.ceil(random_size*epsilon))
...
modeltime_big_m_stengthen_only
...
# Big_M = []
# for i in range(random_size):
#     big_m_coefficient = [0]*knapsack_size
#     for k in range(knapsack_size):
#         big_m_coefficient[k]  = sum(xi[i][k*x_random_size+j] for j in range(x_random_size)) - b[k]
        
#     Big_M.append(max(big_m_coefficient)) 
# # Big_M = [30]*random_size
...
# Initialize Big_M as an empty dictionary
Big_M = {}

for i in range(random_size):
    big_m_coefficient = [0] * knapsack_size
    for k in range(knapsack_size):
        # Calculate the big_m_coefficient for each knapsack constraint 'k'
        big_m_coefficient[k] = sum(xi[i][k * x_random_size + j] for j in range(x_random_size)) - b[k]
    
    # Store the list of big_m_coefficient values for 'i' in the dictionary
    Big_M[i] = big_m_coefficient
...
#     slack_t = m.addVar(lb=0,name="slack_t")
    ...
    m.addConstrs(sum(xi[i][k*x_random_size+j]*x[j] for j in range(x_random_size))  <= b[k]+ Big_M[i][k]*z[i] for k in range(knapsack_size) for i in range(random_size))
...
start=time.time()
model3_bigm = Model3()
obj_big_m = model3_bigm[0]
value_z = model3_bigm[1]
kk=model3_bigm[2]
# obj = 0
# for i in range(x_random_size):
#     obj += c[i]*kk[i]
